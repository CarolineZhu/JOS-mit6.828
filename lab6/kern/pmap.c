...
page_init(void)
{
	...
	/*for (i = 0; i < npages; i++) {
	 ...
	 int occup = (int)ROUNDUP(((char *)envs) + NENV * (sizeof(struct Env)) - KERNBASE, PGSIZE)/PGSIZE;
		...
		if (create)
	 	{
			...
			{
				newpage -> pp_ref++;
				pgdir[d] = page2pa(newpage) | PTE_P | PTE_W | PTE_U;
	 		}
		}
		else
			return NULL;
	 }
	pbase = KADDR(PTE_ADDR(pgdir[d]));
	return (pbase + t);
...
{
	// Fill this function in.
	pte_t *p = pgdir_walk(pgdir, va, ALLOC_ZERO);
	if (!p)
		return -E_NO_MEM;
	pp -> pp_ref++;
	if (((*p) & (~0xFFF)) == page2pa(pp))
		pp -> pp_ref--;
	else if (*p)
	{
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (; start < end; start += PGSIZE)
	{
		...
		if ((int)start > ULIM || cur == NULL || (*cur & PTE_P) == 0 || (*cur & perm) != perm)
		{
			//cprintf("===========================------------===============\n");
			if ((uint32_t)start < (uint32_t)va)
